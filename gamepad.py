...
class GamePadReceiver:
  ...
  def update(self):
    ...
    if has_data:
      self.dpad = result[1]
      self.aLX = self._read_32(result[2], result[3], result[4], result[5])
      self.aLY = self._read_32(result[6], result[7], result[8], result[9])
      self.aRX = self._read_32(result[10], result[11], result[12], result[13])
      self.aRY = self._read_32(result[14], result[15], result[16], result[17])
      self.al2 = self._read_32(result[18], result[19], result[20], result[21])
      self.ar2 = self._read_32(result[22], result[23], result[24], result[25])
    ...
    if self._verbose:
      if ticks_ms() - self._last_print > 100:
        if has_data:
          print('dpad=', self.dpad, ' aX=', self.aLX, ' aY=', self.aLY, ' aRX=', self.aRX, ' aRY=', self.aRY, ' l2=', self.al2, ' r2=', self.ar2, ' buttons=', self.buttons, ' misc=', self.misc_buttons)
        self._last_print = ticks_ms()
